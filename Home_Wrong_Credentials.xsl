...
<!-- Home_Wrong_Credentials.xsl : default home page for application that
			displays error if user entered invalid username-password credentials. -->
		...
		<html>
			...
			<body>	
	            ...
	            <form id="LoginForm" action="verifylogin" method="post" onsubmit="return storeVals(this);">
	             ...
	             <h4 id="InvalidCredentialsError"><xsl:value-of select="root/invalidCredentials" /></h4>
			</body>
		</html>
	</xsl:template>
</xsl:stylesheet>