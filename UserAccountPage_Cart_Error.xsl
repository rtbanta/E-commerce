...
<!-- UserAccountPage_Cart_Error.xsl : displays options available to user in application
			and error signaling user's shopping cart is empty. -->
	...
	<xsl:template match="/">
		<html>
			...
			<body>	
	            ...
	            <h4 id="CartErrorDisplay"><xsl:value-of select="Persons/Person/cartError" /></h4>
			</body>
		</html>
	</xsl:template>
</xsl:stylesheet>